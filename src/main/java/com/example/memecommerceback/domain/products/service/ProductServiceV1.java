 ...
 * 상품 관리를 위한 서비스 인터페이스
 * <p>
 * 상품의 등록, 조회, 수정, 삭제 및 상태 관리 기능을 제공합니다.
 * 사용자 권한(일반 사용자, 판매자, 관리자)에 따른 차별화된 서비스를 제공합니다.
 * </p>
 *
...
public interface ProductServiceV1 {
   ...
   * 새로운 상품을 등록합니다.
   * <p>
   * 상품 정보와 이미지 파일들을 받아 새로운 상품을 시스템에 등록합니다.
   * 상품명과 설명의 유사성 검사, 욕설 필터링, 판매 일정 검증 등을 수행합니다.
   * </p>
   *
   ...
   * 관리자가 상품의 상태를 변경합니다.
   * <p>
   * 관리자 권한으로 상품의 상태를 변경합니다. 
   * 모든 상태 전환이 가능하며, 필요시 판매 일정도 함께 업데이트됩니다.
   * </p>
   *
   ...
   * <p>
   * 판매자가 소유한 상품의 정보를 수정하고 이미지를 변경할 수 있습니다.
   * 상품명과 설명의 유사성 검사, 욕설 필터링, 상태 전환 규칙 검증을 수행합니다.
   * </p>
   *
   ...
   * 상품 상세 정보를 조회합니다.
   * <p>
   * 주어진 상품 ID에 해당하는 상품의 상세 정보를 조회합니다.
   * 상품 정보, 이미지, 좋아요 수, 조회수 등을 포함합니다.
   * </p>
   *
   ...
   * <p>
   * 일반 사용자가 볼 수 있는 상품들(숨김, 대기, 거절 상태 제외)을 
   * 페이지네이션과 정렬 기능과 함께 조회합니다.
   * </p>
   *
   ...
   * <p>
   * 로그인한 판매자가 소유한 모든 상품을 상태에 관계없이 조회할 수 있습니다.
   * 페이지네이션과 정렬, 상태 필터링 기능을 제공합니다.
   * </p>
   *
   * @param page 페이지 번호 (0부터 시작)
   * @param size 페이지당 항목 수
   ...
   * <p>
   * 관리자는 시스템의 모든 상품을 상태에 관계없이 조회할 수 있습니다.
   * 페이지네이션과 정렬, 상태 필터링 기능을 제공합니다.
   * </p>
   *
   * @param page 페이지 번호 (0부터 시작)
   ...
   * 여러 상품을 일괄 삭제합니다.
   * <p>
   * 관리자는 모든 상품을, 판매자는 자신이 소유한 상품만 삭제할 수 있습니다.
   * 상품과 연관된 이미지 파일들도 함께 삭제됩니다.
   * </p>
   *
   ...
   * <p>
   * 스케줄러에 의해 자동으로 호출되는 메소드입니다.
   * 재판매 예정(RESALE_SOON) 상태의 상품 중 판매 시작일이 현재 시간 이전인 상품들을
   * 판매중(ON_SALE) 상태로 변경합니다.
   * </p>
   */
   ...
   * <p>
   * 스케줄러에 의해 자동으로 호출되는 메소드입니다.
   * 판매 종료일이 현재 시간 이전인 상품들을 숨김(HIDDEN) 상태로 변경합니다.
   * </p>
   */
   ...
   * <p>
   * 내부적으로 사용되는 메소드로, 상품 엔티티 객체를 직접 반환합니다.
   * 다른 서비스나 내부 로직에서 상품 엔티티가 필요할 때 사용됩니다.
   * </p>
   *
   * @param productId 조회할 상품의 고유 식별자
   * @return 상품 엔티티 객체
