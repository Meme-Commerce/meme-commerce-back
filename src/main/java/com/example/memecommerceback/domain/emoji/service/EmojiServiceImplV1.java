...
public class EmojiServiceImplV1 implements EmojiServiceV1 {
  ...
  @Override
  @Transactional
  public EmojiResponseDto createOne(
    ...
    if (admin.getNickname() == null) {
      throw new EmojiCustomException(
          EmojiExceptionCode.NEED_TO_REGISTER_NICKNAME);
    }
    Image image = imageService.uploadEmojiImage(multipartFile, admin);
    Emoji emoji = EmojiConverter.toEntity(name, admin, image);
